...
/**
 * 间断性地发送数据给下游 例如我们说知道checkpointedfunction的snatephshate方法在checkpoint的时候可以做sink输出。
 * 但是没法做collect.out()往下游算子输出。 例如：我们有个在做sum聚合的时候不输出，而是往下走做其他trans。但是由于数据量很大，不可能+1就输出一条 例如
 * 100亿条做wordcout，输出从1-100亿都会输出，一条输出一次。太大了，想要做到 定时输出。 snatephshate只能输出到外部系统
 */
public class StreamBatchSendProcessMapFunc extends ProcessFunction<String, String> {
    ...
    public void processElement(String word, Context ctx, Collector<String> out) throws Exception {
        ...
        if (!hasRegisterTimeserver) {
            ctx.timerService()
                    .registerProcessingTimeTimer(System.currentTimeMillis() + batchInterval);

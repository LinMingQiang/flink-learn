...
public class StreamConnectCoProcessFunc
        extends KeyedCoProcessFunction<
                Object, KafkaMessge, KafkaMessge, Tuple3<Boolean, KafkaMessge, KafkaMessge>> {
    // 一对多的场景，v2作为维表
                ...
                getRuntimeContext()
                        .getListState(
                                new ListStateDescriptor(
                                        "test1", TypeInformation.of(KafkaMessge.class)));
        v2 =
                getRuntimeContext()
                        .getState(
                                new ValueStateDescriptor(
                                        "test2", TypeInformation.of(KafkaMessge.class)));
    }

    /**
     * 这里面的watermark是两个共同决定的。 register time server 触发时执行: 清除所有state
     *
     * @param timestamp
     * @param ctx
     * @param out
    ...
    public void onTimer(
            long timestamp,
            OnTimerContext ctx,
            Collector<Tuple3<Boolean, KafkaMessge, KafkaMessge>> out)
            throws Exception {
    ...
    public void processElement1(
            ...
            Collector<Tuple3<Boolean, KafkaMessge, KafkaMessge>> out)
            throws Exception {
        //        System.out.println(value+ "  source wtm : " +
        ...
        if (v2.value() == null) {
            ...
            // 当wartermark超过这个时间的时候就触发ontimer
            ctx.timerService()
                    .registerProcessingTimeTimer(System.currentTimeMillis() + 60000L); // 1分钟后触发
        } else {
    ...
    public void processElement2(
            ...
            Collector<Tuple3<Boolean, KafkaMessge, KafkaMessge>> out)
            throws Exception {
        //        System.out.println(value +  "  v2 wtm : " +
        // ctx.timerService().currentWatermark());
        v2.update(value);
        source.get()
                .forEach(

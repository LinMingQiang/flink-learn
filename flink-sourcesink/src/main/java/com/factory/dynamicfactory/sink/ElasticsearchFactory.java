...
public class ElasticsearchFactory implements DynamicTableSinkFactory {
    ...
    public static final ConfigOption<String> ES_URL =
            key("es.address").stringType().noDefaultValue();

    public static final ConfigOption<String> ES_CLUSTERNAME =
            key("es.clustername").stringType().noDefaultValue();

    public static final ConfigOption<String> ES_PASSW =
            key("es.passw").stringType().noDefaultValue();

    public static final ConfigOption<String> ES_INDEX =
            key("es.index").stringType().noDefaultValue();

    public static final ConfigOption<Long> ES_COMMIT_SIZE =
            key("es.commit.size").longType().noDefaultValue();

    public static final ConfigOption<Long> ES_COMMIT_INTERVAL =
            key("es.commit.intervalsec").longType().noDefaultValue();
    public static final ConfigOption<String> ES_ROWKEY =
            key("es.rowkey").stringType().noDefaultValue();

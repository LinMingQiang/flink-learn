...
public class FlinkStreamCoreSourceSinkTest extends FlinkJavaStreamTableTestBase {
    ...
    //     * 1：
    // 需要再resources/META-INF.services/下创建一个接口名-(org.apache.flink.table.factories.TableFactory)的SPI文件（不是txt）
    //     * 2：在文件里面写上自己实现的类路径
    ...
    //                        tableEnv.sqlQuery("select topic,msg,count(1) as ll from test group by
    // topic,msg")
    //                        , Row.class)
    //                        .filter(x -> x.f0)
    //                        .map(x -> new TestPoJo(x.f1.getField(0).toString(),
    // x.f1.getField(1).toString(), Long.valueOf(x.f1.getField(2).toString())))
    ...
    public void testPrintlneConnect() throws Exception {
        Table a = kafkaDataTable.renameColumns("offset as ll"); // offset是关键字
        tableEnv.createTemporaryView("test", a);
        //        tableEnv.toAppendStream(a, Row.class).print();
        // append
        tableEnv.connect(new PrintlnConnect().property("println.prefix", "connect sink : "))
                .inAppendMode()
        ...
        //                tableEnv.sqlQuery("select topic,msg,count(ll) as ll from test group by
        // topic,msg"));
    ...
    public void testLookupTableSource() throws Exception {
        ...
        //
        //        tableEnv.toAppendStream(
        //                tableEnv.sqlQuery("select * from test, LATERAL TABLE (hbaselookup(id,
        // name))"), Row.class)
        //                .print();

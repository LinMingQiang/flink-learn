...
public class FlinkCoreOperatorTest extends FlinkJavaStreamTableTestBase {
    ...
    public void testWordCount() throws Exception {
        ...
        kafkaDataSource
                .flatMap(
                        (FlatMapFunction<KafkaMessge, Tuple2<String, Long>>)
                                (value, out) -> {
    ...
    public void testOutputTag() throws Exception {
        ...
        SingleOutputStreamOperator<WordCountPoJo> sourceStream =
                kafkaDataSource
                        .process(
                                new KeyedProcessFunction<String, KafkaMessge, WordCountPoJo>() {
                                    @Override
                                    public void processElement(
                                            KafkaMessge value,
                                            Context ctx,
                                            Collector<WordCountPoJo> out) {
                                        if (!value.msg().matches("^[0-9]*$")) {
                                            ctx.output(
                                                    rejectedWordsTag,
                                                    new WordCountPoJo("rejested", 1));
                                        } else {
        ...
        // 正常的数字输出
        sourceStream.keyBy(x -> x.word).sum("num").print();
        // 错误数据输出
        sourceStream.getSideOutput(rejectedWordsTag).keyBy(x -> x.word).sum("num").print();
        System.out.println(streamEnv.getExecutionPlan());
        streamEnv.execute("testOutputTag");
    }

    /**
     * watermark是广播发送的，watermark是广播发送的，watermark是广播发送的 可用于双流join。假设 Test2为维表（永久保存状态） 双流connect
     * 必须都有watermark，否则一个产生不了watermark，不会触发 registerEventimeTimer 输入a: a1 a2 a3 ，a流的wtm = a3
     * 。但是因为是双流 ，所以wtm取最小的 = b = min 再输入b : b2 b3 ， b流的wtm = b3 。 最后的wtm = min (a3, b3) = (a3 - 10s)
     * 这个时候才触发 a的过期，a1,a2 ，如果已经超过10s了的话 后面再输入其他的，就去 最小的那个。每次输入，各自流都会更新自己的wtm，然后再跟另一个比较取最小
     *
     * @throws Exception
     */
    @Test
                        ...
                        new KeyedProcessFunction<String, Tuple2<String, Long>, String>() {
                            ...
                            public void open(Configuration parameters) throws Exception {
                                ValueStateDescriptor desc =
                                        new ValueStateDescriptor<Long>("inc", Types.LONG, 0L);
                            ...
                            @Override
                            public void processElement(
                                    Tuple2<String, Long> value, Context ctx, Collector<String> out)
                                    throws Exception {
                                ctx.timerService()

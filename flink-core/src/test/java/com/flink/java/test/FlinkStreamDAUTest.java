...
public class FlinkStreamDAUTest extends FlinkJavaStreamTableTestBase {
    ...
    /**
     * DAU ： 一天的窗口，每5s输出一次。 不能用滑动窗口， 用翻滚窗口+trigger （翻滚窗口肯定是一天的 0-24，不会有滑动的问题） 当我们需要计算两个报表的时候。 1: day
     * : pv, uv 2: day + msg ： pv，uv 方法1： 开始的时候将一条数据变两条，然后keyby的时候统一用key (但是这样uid也会翻) 方法2： 先按 day+
     * msg去重，然后再 day去重。。。如果报表多了，有点麻烦
     */
    @Test

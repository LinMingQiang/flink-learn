...
public class WordCountTest {
     ...
     * 2：使用yarn-application模式，可以执行多次的execute而不会生成多个applicationid 3：使用sesison可以达到application模式一样的效果
    ...
    public static void runWordCount() {
        DataStreamSource<KafkaManager.KafkaMessge> s1 =
                KafkaSourceManager.getKafkaDataStream(
                        FlinkEvnBuilder.streamEnv,
                        "test",
                        "localhost:9092",
                        "latest",
                        new KafkaMessageDeserialize());
        s1.flatMap(
                        (FlatMapFunction<KafkaManager.KafkaMessge, String>)
                                (value, out) -> {
        ...
        s1.connect(s2)
                ...
                .flatMap(
                        new CoFlatMapFunction<
                                KafkaManager.KafkaMessge, KafkaManager.KafkaMessge, String>() {
                            @Override
                            public void flatMap1(
                                    KafkaManager.KafkaMessge value, Collector<String> out)
                                    throws Exception {
                                for (String s : value.msg().split(",", -1)) {
                                    out.collect(s);
                                }

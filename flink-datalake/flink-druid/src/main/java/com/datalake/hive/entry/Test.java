 ...
 * 只要触发checkpoint就commit，partition暂时不可用（因为watermark不生成） execution.checkpointing.interval: 100000 --
 * ckp触发间隔配置 state.checkpoints.dir: hdfs://ShareSdkHadoop/tmp/flink/checkpoint
 */
public class Test {
    public static void main(String[] args) throws Exception {
        ...
        FlinkLearnPropertiesUtil.init(p, "Flink_Hive_Test");
        // 设置运行模式
        ...
        //          StreamTableEnvironment tableEnv = FlinkEvnBuilder.buildStreamTableEnv(streamEnv,
        // Duration.ofHours(1L));
        ...
        tableEnv.executeSql(
                "INSERT INTO hive_table SELECT app_id, adslot_id, sys_time, DATE_FORMAT(rt, 'yyyy-MM-dd') as dt, DATE_FORMAT(rt, 'HH') as hr"
                        + "  FROM flink_kafka");
    }
}

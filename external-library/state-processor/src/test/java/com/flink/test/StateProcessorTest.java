...
import com.flink.state.processor.api.reader.WordCountJavaPojoKeyreader;
import com.flink.state.processor.api.trans.AccountJavaPojoKeyedStateBootstrapFunction;
import com.flink.state.processor.api.trans.AcountJavaPoJoOperatorStateBootstrap;
...
public class StateProcessorTest extends FlinkJavaStreamTableTestBase {
    ...
    public static String uid = "flatMapUID";
    public static String path = "file:///Users/eminem/workspace/flink/flink-learn/checkpoint";
    public static String newPath = path + "/transCKP";
    public static String CKP_PATH =
            path
                    + "/SocketJavaPoJoWordcountTest/202007061807/e10174e03999d77fb7655a6e9c4f64b4/chk-3";
    /** @throws Exception */
    @Test
    public void testPojoStateProcessor() throws Exception {
        remove(new File(newPath.substring(7, newPath.length())));
        ExistingSavepoint existSp = Savepoint.load(bEnv, CKP_PATH, new RocksDBStateBackend(path));
        DataSet<WordCountPoJo> oldState1 =
                existSp.readKeyedState(uid, new WordCountJavaPojoKeyreader("wordcountState"));
        oldState1.print();
        // 对原始state做转换
        BootstrapTransformation<WordCountPoJo> transformation =
                OperatorTransformation.bootstrapWith(oldState1)
                        // 必须要用 KeySelector 否则报 The generic type parameters of 'Tuple2' are missin
                        .keyBy(
                                new KeySelector<WordCountPoJo, WordCountGroupByKey>() {
                                    @Override
                                    public WordCountGroupByKey getKey(WordCountPoJo value)
                                            throws Exception {
        ...
        existSp.removeOperator(uid).withOperator(uid, transformation).write(newPath);
        bEnv.execute("jel");

        Savepoint.load(bEnv, newPath, new RocksDBStateBackend(path))
                .readKeyedState(uid, new WordCountJavaPojoKeyreader("wordcountState"))
                .print();
    }
    ...
    public void testOpearteStateProcessor() throws Exception {
        String sourcePath =
                path
                        + "/SocketJavaPoJoWordcountTest/202007071207/fe4dfe843f45d4bb61b4b0a6a4307d68/chk-2";
        remove(new File(newPath.substring(7, newPath.length())));
        ExistingSavepoint existSp = Savepoint.load(bEnv, sourcePath, new RocksDBStateBackend(path));
        // read src key state
        DataSet<WordCountPoJo> oldState1 =
                existSp.readListState(
                        "wordcountsink", "opearatorstate", Types.POJO(WordCountPoJo.class));
        oldState1.print();
        // trans
        existSp.removeOperator("wordcountsink")
                .withOperator(
                        "wordcountsink",
                        OperatorTransformation.bootstrapWith(oldState1)
        ...
        // read new key state
        Savepoint.load(bEnv, newPath, new RocksDBStateBackend(path))
                .readListState("wordcountsink", "opearatorstate", Types.POJO(WordCountPoJo.class))
                .print();
    }
    ...
    public void testKeyAndOperatorState() throws Exception {
        ...
        oldState1.print();
        // key state
        DataSet<WordCountPoJo> keysState =
                existSp.readKeyedState(uid, new WordCountJavaPojoKeyreader("wordcountState"));
        keysState.print();

        // key state trans
        BootstrapTransformation<WordCountPoJo> keystateTransformation =
                OperatorTransformation.bootstrapWith(oldState1)
                        // 必须要用 KeySelector 否则报 The generic type parameters of 'Tuple2' are missin
                        .keyBy(
                                new KeySelector<WordCountPoJo, WordCountGroupByKey>() {
    ...
    //        remove(new File(newPath.substring(7, newPath.length())));
    //        ExistingSavepoint existSp = Savepoint.load(bEnv, sourcePath, new
    // RocksDBStateBackend(path));
    ...
    //        bEnv.execute("jel");
    //
    //        ExistingSavepoint existSp2 = Savepoint.load(bEnv, newPath, new
    // RocksDBStateBackend(path));

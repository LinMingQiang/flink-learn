...
public class TestMonitor {
    ...
    public void testHandler() throws Exception {
        ...
        r.forEach(
                x -> {
                    try {
                        List<FlinkApplicationJobsInfo> jobInfos =
                                yarnclient.getFlinkJobsOverview(x.id);
                        FlinkApplicationJobsInfo jobInfo = jobInfos.get(0);
                        FlinkJobsExceptionInfo jobsExcepInfo =
                                yarnclient.getFlinkJobExceptions(x.id, jobInfo.jid);
                        System.out.println(jobsExcepInfo);
                        FLinkJobsCheckpointInfo ckpInfo =
                                yarnclient.getFlinkJobCheckpoint(
                                        x.id, yarnclient.getFlinkJobsOverview(x.id).get(0).jid);
                        System.out.println(ckpInfo.latest);

                        // tm的内存信息
                        //                List<FlinkTaskManagerInfo> flinkAllTmInfos =
                        // yarnclient.getFlinkJobAllTasksManagersInfo(x.id);
                        //                flinkAllTmInfos.forEach(tminfo -> {
                        //                    try {
                        //                        FlinkJobTaskManagerInfo tmin =
                        // yarnclient.getFLinkTaskManagerInfo(x.id, tminfo.id);
    ...
    public void testHttp() throws IOException {
        JSONObject json =
                JSON.parseObject(
                        OkHttp3Client.get(
                                "http://10-21-129-142-jhdxyjd.mob.local:10880/ws/v1/cluster/apps?state=RUNNING"));
    ...
    public void testYarnAllappInfo() throws IOException, YarnException {
        YarnRestFulClient yarnclient =
                YarnRestFulClient.getInstance("http://10-21-129-141-jhdxyjd.mob.local:10880");
    ...
    public void testFlinkjobs() throws IOException, YarnException {
        YarnRestFulClient yarnclient =
                YarnRestFulClient.getInstance("http://10-21-129-141-jhdxyjd.mob.local:10880");
        Map<String, List<FlinkApplicationJobsInfo>> m =
                yarnclient.getFlinkAllApplicationJobsInfo("RUNNING");
    ...
    public void testFlinkAllTaskmanager() throws IOException, YarnException {
        ...
        String appid = r.get(0).id;
        yarnclient
                .getFlinkJobAllTasksManagersInfo(appid)
                .forEach(
                        x -> {
    ...
    public void testFLINK_JOBS_JID_INFO() throws IOException, YarnException {
        ...
        yarnclient
                .getFlinkJobJidInfo(appid, jobid)
                .vertices
                .forEach(
                        x -> {

...
public class FlinkSourceBuilder extends FlinkStreamEnvAndSource {
    ...
    public static void init() throws IOException {
        FlinkLearnPropertiesUtil.init(EnvironmentalKey.LOCAL_PROPERTIES_PATH(), "local-test");
        streamEnv =
                FlinkEvnBuilder.buildStreamingEnv(
                        FlinkLearnPropertiesUtil.param(),
                        FlinkLearnPropertiesUtil.CHECKPOINT_PATH(),
                        FlinkLearnPropertiesUtil.CHECKPOINT_INTERVAL());
        tableEnv = FlinkEvnBuilder.buildStreamTableEnv(streamEnv, Duration.ofMinutes(1));
    ...
    public static void init(
            ...
            Duration stateTTL)
            throws IOException {
        FlinkLearnPropertiesUtil.init(prop_path, proName);
        streamEnv =
                FlinkEvnBuilder.buildStreamingEnv(parameters, checkpointPath, checkPointInterval);
        tableEnv = FlinkEvnBuilder.buildStreamTableEnv(streamEnv, stateTTL);
        kafkaDataSource = getKafkaKeyStream("test", "localhost:9092", "latest");
        kafkaDataTable = getStreamTable("test", "localhost:9092", "latest");
    }
    /**
    ...
    public static KeyedStream<KafkaMessge, String> getKafkaKeyStream(
        ...
        return getKafkaDataStreamSource(topic, broker, reset)
                .setParallelism(3)
                .keyBy(
                        new KeySelector<KafkaMessge, String>() {
                            @Override
                            public String getKey(KafkaMessge value) throws Exception {
                                return value.msg();
                            }
                        });
    }
    ...
    public static SingleOutputStreamOperator<KafkaMessge> getKafkaDataStreamSource(
            String topic, String broker, String reset) {
        return streamEnv
                .addSource(getKafkaConsumer(topic, broker, reset))
     ...
     * @return
     */
    public static FlinkKafkaConsumer<KafkaMessge> getKafkaConsumer(
            String topic, String broker, String reset) {
        FlinkKafkaConsumer<KafkaMessge> kafkasource =
     ...
     * @return
     */
    protected static Table getStreamTable(
            KeyedStream<KafkaMessge, String> source, Expression... fields) {
    ...
    // 获取默认表
    public static Table getStreamTable(String topic, String broker, String reset) {
        return getStreamTable(
                getKafkaKeyStream(topic, broker, reset),
                $("topic"),
                $("offset"),
                $("ts"),
                $("msg"),
    ...
    public static Table getStreamTable(
            String topic, String broker, String reset, Expression... fields) {

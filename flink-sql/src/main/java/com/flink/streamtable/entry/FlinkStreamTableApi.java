    ...
    public static void runTemJoin() {
        ...
        Table ratesHistory =
                getStreamTable(
                        ...
                        $("msg"),
                        $("ts").rowtime()); // 提供一个汇率历史记录表静态数据集

        printlnStringTable(ratesHistory);
        printlnStringTable(orders);

        TemporalTableFunction rates = ratesHistory.createTemporalTableFunction($("ts"), $("msg"));
        tableEnv.createTemporarySystemFunction("rates", rates);

        Table result =
                orders.joinLateral(call("rates", $("o_rowtime")), $("o_currency").isEqual($("msg")))
                        .select($("o_rowtime"));

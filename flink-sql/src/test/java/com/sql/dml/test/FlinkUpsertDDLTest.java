...
public class FlinkUpsertDDLTest extends FlinkJavaStreamTableTestBase {
    ...
    public void upsertKafkaSinkTest() throws ExecutionException, InterruptedException {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createUpsertKafkaSinkTable(
    ...
    /**
     * upsert-kafka 只支持从 earliest开始消费 （其实输入数据都是 I和D） // 查看执行计划就能看懂了，多了一个 ChangelogNormalize
     * 用来处理输入的数据，
     *
     * @throws ExecutionException
     * @throws InterruptedException
    ...
    public void upsertKafkaSourceTest() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createUpsertKafkaSinkTable(
                        "localhost:9092", "test2", "test", "test", "json"));
        tableEnv.executeSql(DDLSourceSQLManager.createDynamicPrintlnRetractSinkTbl("test2"));
    ...
    /**
     * mysql -uroot -p123456 CREATE TABLE t1( msg char(255) not null primary key, cnt BIGINT(20) );
     * sink的时候配置了批次和间隔后，对数据的缓存是按key去重之后最后再写出，同一个key只保留最新的记录 {"rowtime":"2020-01-01
     * 00:00:01","msg":"c"}
     */
    @Test

...
public class FlinkCDCTest extends FlinkJavaStreamTableTestBase {
    ...
    // INSERT INTO `order_info` VALUES (11, 'iwKjQD', '13320383859', 88.00, '1', 107, '1',
    // 'cbXLKtNHWOcWzJVBWdAs', 'njjsnknHxsxhuCCeNDDi', '0937074290', '', '2020-06-18 15:56:34',
    ...
    public void cdcTest() throws Exception {
        ...
        tableEnv.toRetractStream(tableEnv.from("order_info"), Row.class)
                .filter(
                        (FilterFunction<Tuple2<Boolean, Row>>)
                                booleanRowTuple2 -> booleanRowTuple2.f0)
                .map(
                        (MapFunction<Tuple2<Boolean, Row>, Tuple2<String, String>>)
                                booleanRowTuple2 ->
                                        new Tuple2<>(
                                                booleanRowTuple2.f1.getField("id").toString(),
                                                booleanRowTuple2.f1.getField("id").toString()))
                .returns(Types.TUPLE(Types.STRING, Types.STRING))
                .assignTimestampsAndWatermarks(
                        WatermarkStrategy.<Tuple2<String, String>>forBoundedOutOfOrderness(
                ...
                .reduce(
                        (ReduceFunction<Tuple2<String, String>>)
                                (v1, v2) ->
                                        new Tuple2(
                                                v1.f0,
                                                String.valueOf(
                                                        Long.valueOf(v1.f1) + Long.valueOf(v2.f1))),
                        new ProcessWindowFunction<
                            ...
                            public void process(
                                    ...
                                    Collector<Tuple2<TimeWindow, Long>> out)
                                    throws Exception {
                                Long count = 0L;
        ...
        tableEnv.toRetractStream(tableEnv.from("order_info"), Row.class).print();
    ...
    // UPDATE `order_info` SET consignee='1',create_time = '2020-06-18 20:00:15',img_url='15' where
    // id = 1;
    // 需要两边的wtm才能触发
    @Test

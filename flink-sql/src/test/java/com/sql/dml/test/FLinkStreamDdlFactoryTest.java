...
//        set.addInsertSql("insert into printSink select msg,max(1) cnt from test group by msg");
    ...
    public void mongoTest() {
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test;test2", "test", "test", "json"));
        ...
        //        TableResult re = tableEnv.executeSql("insert into mongotest select msg as
        // id,msg,count(1) uv from test3 group by msg");
        re.print();
    }
    ...
    public void clickhouseSinkTest() {
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test", "test2", "test", "json"));
        tableEnv.executeSql(DDLSourceSQLManager.createCustomClickhouseSink("test"));
        TableResult re =
                tableEnv.executeSql(
                        "insert into test select count(1) as id,msg as name from test2 group by msg");
        re.print();
    }
    ...
    public void temporalTableTest() throws Exception {
        ...
        tableEnv.toAppendStream(
                        tableEnv.sqlQuery(
                                "select * from test t1 JOIN hbaselookup FOR SYSTEM_TIME AS OF t1.proctime as t2 ON t1.msg = t2.word"
                                        + ""),
                        Row.class)
                .print();
        streamEnv.execute();
    ...
    public void UDTFTest() throws Exception {
        ...
        tableEnv.toAppendStream(
                        tableEnv.sqlQuery(
                                "select * from"
                                        + " test LEFT JOIN"
                                        + " LATERAL TABLE(split(msg)) AS t(word, word1) ON TRUE"),
                        Row.class)
                .print();
    ...
    // 时态表函数
    // 不能在ddl中定义时态表函数
    // 时态表函数和时态表 DDL 最大的区别在于，时态表 DDL 可以在纯 SQL 环境中使用但是时态表函数不支持，用时态表 DDL 声明的时态表支持 changelog 流和
    // append-only 流但时态表函数仅支持 append-only 流。
    @Test
    public void temporalTableFunctionTest() throws Exception {
        ...
        TemporalTableFunction rates =
                tableEnv.from("test2")
                        .renameColumns($("msg").as("r_msg"), $("proctime").as("r_proctime"))
                        .createTemporalTableFunction($("r_proctime"), $("r_msg"));
    ...
    // 时态表分： 版本表和普通表
    // FlinkCDCTest
    // 版本表: 如果时态表中的记录可以追踪和并访问它的历史版本，这种表我们称之为版本表，来自数据库的 changelog
    // 可以定义成版本表。1：使用cdc，或者debezium-json格式的kafka数据。
    // 普通表: 如果时态表中的记录仅仅可以追踪并和它的最新版本，这种表我们称之为普通表，来自数据库 或 HBase
    // 的表可以定义成普通表。2：使用LookupableTableSource自己实现
    ...
    public void temporalTableJoinTest() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createTemporalTable(
                        "localhost:9092",
                        "test2",
                        "test2",
                        "test",
    ...
    /**
     * 定义一个Row(xx Long, yy Int) 的类型，用来将指标统一在一个字段里面，方便 写出的时候用 {"rowtime":"2020-06-18
     * 16:59:30","msg":"1"}
     */
    @Test
    public void sqlRowTypeTest() throws Exception {
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test", "test", "test", "json"));
        String sql =
                "select msg as id,msg,Row(count(1), sum(`offset`)) as inc from test group by msg";
        ...
        //                    @Override
        //                    public String map(Tuple2<Boolean, Row> booleanRowTuple2) throws
        // Exception {
    ...
    public void testBigintToTimeStamp3() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafkaRowtime(
                        "localhost:9092", "test", "test", "test", "json"));

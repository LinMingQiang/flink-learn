...
public class FlinkJiraBugTest extends FlinkJavaStreamTableTestBase {
    ...
    //        tableEnv.createTemporaryView("test"
    // ,tableEnv.fromDataStream(streamEnv.fromElements("a", "b", "c", "d", "e"), $("msg")));
    ...
    //
    //        tableEnv.toAppendStream(tableEnv.sqlQuery("select * from tmp where c > 5"),
    // Row.class).print();
    ...
    //        tableEnv.createTemporaryView("test"
    // ,tableEnv.fromDataStream(streamEnv.fromElements("a", "", ""), $("msg")));
    ...
    /**
     * 在流表转换中的时间字段的定义 正常情况下，我们要定义rowtime和watermark （只有窗口中用到）。我们只能在ddl。或者在TableApi的时候定义
     * 但是如果我们有一个Table里面带有时间字段，但他不是rowtime，这个表我们是无法使用window操作的。 例如我们有个table是经过多个table转换过来的。 1:
     * 将table转stream 2：在stream中定义watermark 3：stream转table，同时指定rowtime 注意：只要对表或者stream做了groupby就会导致
     * wtm不生成导致窗口无法触发。 注意： 简单的表转换是不会丢失时间信息的。只要不变换时间字段
    ...
    public void stream2TableWatermarkErr() throws Exception {
        ...
        tableEnv.createTemporaryView(
                "test2", tableEnv.sqlQuery("select msg,rowtime from test group by msg,rowtime"));

        SingleOutputStreamOperator r =
                tableEnv.toRetractStream(tableEnv.from("test2"), Row.class)
                        //                        .keyBy((KeySelector<Tuple2<Boolean, Row>, String>)
                        // value -> value.f1.getField(0).toString())
                        .map(
                                new MapFunction<Tuple2<Boolean, Row>, Tuple2<String, Long>>() {
                                    SimpleDateFormat s =
                                            new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");

                                    @Override
                                    public Tuple2<String, Long> map(Tuple2<Boolean, Row> value)
                                            throws Exception {

...
public class FlinkSQLExplainSrcRead extends FlinkJavaStreamTableTestBase {
    ...
    public void sqlExplainTest() {
        ...
        System.out.println(tableEnv.explainSql(selectSQL));
        //        set.addInsertSql("insert into mysqltest select msg,count(1) cnt from test group by
        // msg");
        //        set.execute();
    ...
    /** local agg 依赖于 mini batch 执行计划在 StreamPhysicalAggRule那再转一次，但是加上了localAgg */
    @Test
    ...
    public void windowTest() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test", "test", "test", "json"));
        String selectSQL =
                "SELECT window_start,window_end,msg,COUNT(1) AS sellCount "
        ...
        //        +- WindowAggregate(groupBy=[msg], window=[TUMBLE(time_col=[proctime], size=[10
        // s])], select=[msg, COUNT(*) AS sellCount, start('w$) AS window_start, end('w$) AS
        // window_end])
        ...
        // watermark=[-($0, 10000:INTERVAL SECOND)]]], fields=[rowtime, msg, uid, topic, offset])
    }
    ...
    public void miniBatchHLLResultIsZeroErr() throws Exception {
        ...
        configuration.setString(
                ...
                "TWO_PHASE"); // enable two-phase, i.e. local-global aggregation
        tableEnv.executeSql(
                "CREATE FUNCTION hll_distinct AS 'com.flink.learn.sql.func.HyperLogCountDistinctAgg'");

        tableEnv.executeSql(DDLSourceSQLManager.createDynamicPrintlnRetractSinkTbl("mysqlSinkTbl"));
        System.out.println(
                tableEnv.explainSql(
                        "insert into mysqlSinkTbl SELECT msg,hll_distinct(msg) cnt from test group by msg"));
        StatementSet set = tableEnv.createStatementSet();
        set.addInsertSql(
                "insert into mysqlSinkTbl SELECT msg,hll_distinct(msg) cnt from test group by msg");
        set.execute().await();

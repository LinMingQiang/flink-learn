...
public class FlinkStreamTableDdlTest extends FlinkJavaStreamTableTestBase {
    ...
    /**
     * 在main里面可以执行，不需要 execute 在test里面执行得用TableResult
     *
     * @throws Exception
     */
    @Test
    public void testDDLSample() throws Exception {
        ...
        //        tableEnv.toRetractStream(tableEnv.sqlQuery("select concat(msg, topic) as ss,max(*)
        // as cnt from test group by msg,topic"), Row.class)
        //                .print();
        //        // 要在 executeSql 之前
    ...
    public void testDDLWindow() throws Exception {
        // {"rowtime":"2021-01-20 00:00:01","msg":"hello"} {"rowtime":"2021-01-20
        // 00:00:02","msg":"hello"}
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test2", "test2", "test", "json"));
        tableEnv.executeSql(
                DDLSourceSQLManager.createDynamicPrintlnRetractSinkTbl("printlnRetractSink"));
    ...
    public void testOverWindow() throws Exception {
        ...
        // TUMBLE_ROWTIME 返回的字段做为 rowtime
        String sql =
                "select count(distinct `offset`) over w,sum(`offset`) over w "
                        + "FROM test "
                        + "window w AS ("
                        + "partition by msg "
                        + "order by rowtime "
    ...
    public void testIntervalJoin() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test2", "test2", "test2", "json"));
        // o-4 < s < o
        // o < s < o+4
        // o-1 < s < o+3
        // TUMBLE_ROWTIME 返回的字段做为 rowtime
    ...
    public void testCustomJsonFormat() throws Exception {
        ...
        tableEnv.executeSql(
                DDLSourceSQLManager.createStreamFromKafka(
                        "localhost:9092", "test", "test", "test", "custom-json"));
        tableEnv.executeSql(
                DDLSourceSQLManager.createDynamicPrintlnRetractSinkTbl("printlnRetractSink"));
        // TableResult re = tableEnv.executeSql("insert into printlnRetractSink select msg , 1 as
        // cnt from test");
    ...
    public void testDDLSample2() throws Exception {
        ...
        String sql =
                "select msg, count(1) over w as w_c,sum(`offset`) over w as w_s "
                        + "FROM test "
                        + "window w AS ("
                        + "partition by msg "
                        + "order by rowtime "
        ...
        //        TableResult re = tableEnv.executeSql("insert into printlnRetractSink select
    ...
    /**
     * 需要再sql的 WindowOperator 里面去修改 必须是insert xx select... emit
     *
     * @throws Exception
     */
    @Test
    public void testDDLTriggerWindow() throws Exception {
        // {"rowtime":"2021-01-20 00:00:00","msg":"hello"} {"rowtime":"2021-01-20
        // 00:00:02","msg":"hello"}
        // {"rowtime":"2021-01-20 00:02:50","msg":"hello"}
        //
    ...
    public void hyperlogCountdistinctTest() throws Exception {
        ...
        tableEnv.createTemporarySystemFunction(
                "hyperCountDistinct", new HyperLogCountDistinctAgg());
        tableEnv.executeSql(
                DDLSourceSQLManager.createDynamicPrintlnRetractSinkTbl("printlnRetractSink"));
        //        String sql = "select " +
        //                "msg," +
    ...
    public void streamToTableTimeAttributesTest() throws Exception {
        ...
        tableEnv.createTemporaryView(
                "test2",
                tableEnv.sqlQuery(
                        "select CONCAT(msg , '-hai') as msg,rowtime from test where msg is not null"));
        //              tableEnv.createTemporaryView("test2", tableEnv.sqlQuery("select msg,rowtime
        // from (select msg,rowtime from test) union all (select msg,rowtime from test)"));
        // 无法触发
        //            tableEnv.createTemporaryView("test2", tableEnv.sqlQuery("select msg,rowtime
        // from test group by msg,rowtime"));
        // 1: table转stream。同时指定 wtm的时间抽取
        SingleOutputStreamOperator r =
                tableEnv.toRetractStream(tableEnv.from("test2"), Row.class)
                        .filter(x -> x.f0)
                        // 加了keyby就无法触发
                        //                .keyBy((KeySelector<Tuple2<Boolean, Row>, String>) value
                        // -> value.f1.getField(0).toString())
                        .map(
                                new MapFunction<Tuple2<Boolean, Row>, Tuple2<String, String>>() {
                                    SimpleDateFormat s =
                                            new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");

                                    @Override
                                    public Tuple2<String, String> map(Tuple2<Boolean, Row> value)
                                            throws Exception {
                                        String formatstr = value.f1.getField(1).toString();
                                        if (formatstr.length() < 19) formatstr += ":00";
                                        return new Tuple2<>(
                                                value.f1.getField(0).toString(), formatstr);
                                    }
                                })
                        // 这个wtm的定义可以在.watermark("rowtime", "rowtime - INTERVAL '10' SECOND")
                        // 定义了，1.13版本
    ...
    /** 测试当数据源做union的时候wtm生成 结果：选的是两个stream最小的做wtm */
    @Test
    ...
    public void windowTVF() {
        ...
        //        String sql = "select * " +
        //                " from TABLE(TUMBLE(TABLE test,DESCRIPTOR(rowtime), INTERVAL '30'
        // SECOND))";
        ...
        //                "msg," +
        //                "count(1) cnt" +
        //                " from TABLE(HOP(TABLE test, DESCRIPTOR(rowtime), INTERVAL '5' SECOND,
        // INTERVAL '10' SECOND))" +
        ...
        //                "msg," +
        //                "count(uid) cnt" +
        //                " from TABLE(CUMULATE(TABLE test,DESCRIPTOR(rowtime), INTERVAL '30'
